...
const doubleModal: FC<doubleModalProps> = ({ show, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg border bg-opacity-80 border-black-400 rounded-[20px] shadow-xl overflow-hidden max-w-sm w-full md:max-w-md lg:max-w-lg" onClick={e => e.stopPropagation()}>
      ...
      <div className="bg-white rounded-lg border bg-opacity-80 border-black-400 shadow-xl overflow-hidden max-w-sm w-full md:max-w-md lg:max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="bg-none p-3 flex justify-between items-center">
          <div>
          </div>
