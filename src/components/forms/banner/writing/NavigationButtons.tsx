...
const NavigationButtons: React.FC<NavigationButtonsProps> = ({ isListening, setIsListening }) => (
  <div className="flex justify-between items-center">
    <Link href={`/`} passHref>
      <button className="btn btn-outline xl:btn-lg font-bold border-2">뒤로 가기</button>
    </Link>
    <button
    ...
    <Link href={`/feedback/waiting`} passHref>
      <button className="btn btn-outline xl:btn-lg btn-success font-bold border-2">보내기</button>
    </Link>
  </div>
);
